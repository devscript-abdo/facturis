/*!
 * Facturis - v5.5.0
 * @author Facturis - https://facturis.ma
 * Copyright (c) 2023
 */
...
export default class Template {
  ...
  _uiHandleDarkMode(mode = 'init') {
    ...
    if (mode === 'init' && !self._lPage.classList.contains('dark-mode')) {
      if (self._lPage.classList.contains('sidebar-dark')) {
        localStorage.setItem('FacturisDefaultsSidebarDark', true);
      } else {
        localStorage.removeItem('FacturisDefaultsSidebarDark');
      }

      if (self._lPage.classList.contains('page-header-dark')) {
        localStorage.setItem('FacturisDefaultsPageHeaderDark', true);
      } else {
        localStorage.removeItem('FacturisDefaultsPageHeaderDark');
      }
    }

    // If remember-theme class is added in #page-container
    if (self._lPage.classList.contains('remember-theme')) {
      let darkMode = localStorage.getItem('FacturisDarkMode') || false;
      ...
      if (mode === 'init') {
        ...
        localStorage.setItem('FacturisDarkMode', true);
      } else if (mode === 'off') {
        localStorage.removeItem('FacturisDarkMode');
      }
    } else if (mode === 'init') {
      localStorage.removeItem('FacturisDarkMode');
    }
  ...
  _uiHandleTheme() {
    ...
    if (rememberTheme) {
      let themeName = localStorage.getItem('FacturisThemeName') || false;
      ...
      localStorage.removeItem('FacturisThemeName');
    }
    ...
    document.querySelectorAll('[data-toggle="theme"]').forEach(el => {
      el.addEventListener('click', e => {
        ...
        if (rememberTheme) {
          localStorage.setItem('FacturisThemeName', themeName);
        }
      });
    });
  }
  ...
  _uiApiLayout(mode = 'init') {
    ...
    let layoutAPI = {
      ...
      dark_mode_off: () => {
        if (!localStorage.getItem('FacturisDefaultsSidebarDark')) {
          self._lPage.classList.remove('sidebar-dark');
        }

        if (!localStorage.getItem('FacturisDefaultsPageHeaderDark')) {
          self._lPage.classList.remove('page-header-dark');
        }
